	...
	public static HexPosition[] search (UnitTest unit, HexPosition start, HexPosition goal, int max, int distance=0) {
		...
		while (fScore.Count > 0) {
			...
			foreach(HexPosition neighbor in current.Neighbors) {
				int moveTime = unit.getMoveTime(neighbor);
				if(moveTime < 0 || neighbor.containsKey("Unit")) {	//TODO: If you want to change what's impassible, you'd change it here.
					continue;
				}
