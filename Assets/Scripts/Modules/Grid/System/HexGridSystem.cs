...
namespace BattlefieldSimulator
{
    public class HexGridSystem
    {
        ...
        public HexGridMapData Data { get => data; }
        ...
        public bool FindPath(HexTile origin, HexTile destination, out List<HexTile> path, bool isFlight)
        {
            path = Pathfinder.FindPath(origin, destination,
                new Vector2(generatorData.topografiaSettings.seaLevel, generatorData.topografiaSettings.heightScale), isFlight);
        ...
        public Vector3 GetLocalPositionForHexFromCoordinate(GridPosition gridPosition)
        {
            ...
            {
                return hexTile.gameObject.transform.position;
            }
            else
            {
                yPosition = 0;
